...
class ModelCatalogFeedback extends Model
{
    public function addfeedback($data)
    {
        ...
        foreach ($data['feedback_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO " . DB_PREFIX . " 
                feedback_description SET feedback_id = '" . (int)$feedback_id . "', 
                language_id = '" . (int)$language_id . "', 
                feedback_author= '" . $this->db->escape($value['feedback_author']) . "', 
    ...
    {
        $this->db->query("UPDATE " . DB_PREFIX . "feedback SET
       sort_order = '" . (int)$data['sort_order'] . "', status = '" .
    ...
    public function getfeedback($feedback_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX .
            "feedback WHERE feedback_id = '" . (int)$feedback_id . "'");
        return $query->row;
    }
    ...
    public function getfeedbacks($data = array())
    {
        if ($data) {
            $sql = "SELECT * FROM " . DB_PREFIX . "feedback f LEFT JOIN "
                . DB_PREFIX . "feedback_description fd ON (f.feedback_id =
                                fd.feedback_id) WHERE fd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
            $sort_data = array('fd.feedback_author', 'f.sort_order');
            if (isset($data['sort']) && in_array($data['sort'],
                    $sort_data)) {
                $sql .= " ORDER BY " . $data['sort'];
            } else {
                $sql .= " ORDER BY fd.feedback_author";
            }
            ...
            if (!$feedback_data) {
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX .
                    "feedback f LEFT JOIN " . DB_PREFIX . "feedback_description
            fd ON (f.feedback_id = fd.feedback_id) WHERE fd.language_id
